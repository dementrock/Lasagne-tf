...
def collect_shared_vars(expressions):
    ...
    if theano.compat.is_variable(expressions):
        expressions = [expressions]
    # return list of all shared variables
    return [v for v in theano.compat.get_inputs(reversed(expressions))
            if theano.compat.is_shared(v)]
...
def create_param(spec, shape, name=None):
    ...
    if theano.compat.is_variable(spec):
